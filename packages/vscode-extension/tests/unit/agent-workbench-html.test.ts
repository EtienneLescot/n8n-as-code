...
test('CLI skills assets: extension bundles only runtime-required agent assets', () => {
    ...
    // CLI cannot drift apart on where assets live. The guard follows the implementation.
    ...
    assert.ok(skillsCliSource.includes('resolveSkillsAssetsDir'), 'Direct skills CLI must resolve assets through the shared resolver, not its own copy');
