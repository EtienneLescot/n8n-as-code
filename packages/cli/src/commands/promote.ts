...
export class PromoteCommand {
    ...
    async run(sourceWorkflowPath: string, options: PromoteOptions): Promise<PromoteResult> {
        const source = this.configService.resolveEnvironment(options.from);
        const target = this.configService.resolveEnvironment(options.to);
        ...
        if (!this.isPathInside(sourcePath, sourceRoot)) {
        ...
        if (fs.existsSync(targetPath) && !options.overwrite) {
            throw new Error(`Target workflow already exists: ${targetPath}. Use --overwrite to replace it.`);
        }
    ...
    private getEnvironmentWorkflowRoot(environment: IResolvedWorkspaceEnvironment): string {
        return path.resolve(environment.workflowDir || environment.syncFolder);
    }
...
export function adaptWorkflowForPromotion(content: string): string {
