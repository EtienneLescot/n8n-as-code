...
For troubleshooting, run `npx --yes n8nac skills list --debug` to print the resolved `n8nac-config.json`
path, the selected custom nodes file, and the merged node counts. If you're checking a single
custom node, `npx --yes n8nac skills node-info <nodeName> --debug` is the most direct verification.
