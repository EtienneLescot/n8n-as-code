...
npx n8nac@v1 skills <command>

# Or install globally
npm install -g n8nac@v1
```

Note: When you run `update-ai` from the main `n8nac`, ensure V1 is available to the project (install locally with `npm install --save-dev n8nac@v1`, install globally, or use `npx n8nac@v1`). The VS Code extension is the only caller that generates the AGENTS.md and AI context files.
...
2. AI runs: npx n8nac@v1 skills search "google sheets"
3. AI gets node schemas: npx n8nac@v1 skills node-info googleSheets
...
1. **Search for nodes you need:**
   ```bash
   npx n8nac@v1 skills search "your query"
   ```

2. **Get detailed schema for a specific node:**
   ```bash
   npx n8nac@v1 skills node-info nodeName
   ```

3. **List all available nodes:**
   ```bash
   npx n8nac@v1 skills list
   ```
...
npx n8nac@v1 skills list | grep "your-node"
```

**JSON parsing issues:**
```bash
# Pipe output to jq for pretty printing
npx n8nac@v1 skills search "http" | jq .
```
