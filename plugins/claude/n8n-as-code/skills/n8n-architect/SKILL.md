...
- **NEVER tell the user to run `npx --yes n8nac@v1 init` themselves.** You are the agent — it is YOUR job to run the command.
- For autonomous agents, the default non-interactive initialization flow is the explicit 2-step sequence: `npx --yes n8nac@v1 init-auth --host <url> --api-key <key> [--sync-folder <path>]`, then `npx --yes n8nac@v1 init-project --project-id <id>|--project-name <name>|--project-index <n> [--sync-folder <path>]`. Use this when the project is not known yet and you need to discover or inspect projects before choosing one.
- A 1-command non-interactive flow also exists when the host, API key, and project selector are already known: `npx --yes n8nac@v1 instance add --yes --host <url> --api-key <key> --project-id <id>|--project-name <name>|--project-index <n> [--sync-folder <path>]`. `npx --yes n8nac@v1 init` is the ergonomic alias.
- If the workspace already has saved instance configs, inspect them with `npx --yes n8nac@v1 instance list --json` before deciding whether to add a new one or switch the active config.
- Use `npx --yes n8nac@v1 instance select --instance-id <id>` or `npx --yes n8nac@v1 instance select --instance-name <name>` to switch saved configs non-interactively.
- Use `npx --yes n8nac@v1 instance delete --instance-id <id> --yes` or `npx --yes n8nac@v1 instance delete --instance-name <name> --yes` to remove stale saved configs non-interactively.
...
- Default 2-step non-interactive auth: `npx --yes n8nac@v1 init-auth --host <url> --api-key <key> [--sync-folder <path>]`
- Default 2-step non-interactive project selection: `npx --yes n8nac@v1 init-project --project-id <id>|--project-name <name>|--project-index <n> [--sync-folder <path>]`
- Optional 1-command non-interactive setup when the project is already known: `npx --yes n8nac@v1 instance add --yes --host <url> --api-key <key> --project-id <id>|--project-name <name>|--project-index <n> [--sync-folder <path>]`
- Optional 1-command alias: `npx --yes n8nac@v1 init --yes --host <url> --api-key <key> --project-id <id>|--project-name <name>|--project-index <n> [--sync-folder <path>]`
- Saved config management: `npx --yes n8nac@v1 instance list --json`, `npx --yes n8nac@v1 instance select --instance-id <id>|--instance-name <name>`, `npx --yes n8nac@v1 instance delete --instance-id <id>|--instance-name <name> --yes`
- `npx --yes n8nac@v1 init-project` can run interactively after `npx --yes n8nac@v1 init-auth`, or non-interactively when the project selector is known.

### Required Order
1. Check for `n8nac-config.json`.
2. If saved configs already exist: inspect them with `npx --yes n8nac@v1 instance list --json`. Reuse them with `npx --yes n8nac@v1 instance select` instead of creating duplicates whenever that satisfies the user request.
3. If initialization is missing and `N8N_HOST` / `N8N_API_KEY` are available: default to `npx --yes n8nac@v1 init-auth --host <url> --api-key <key> [--sync-folder <path>]` to discover projects. Only use `npx --yes n8nac@v1 instance add --yes --host <url> --api-key <key> --project-id <id>|--project-name <name>|--project-index <n> [--sync-folder <path>]` when the project is already known.
...
5. After credentials are saved, inspect the listed projects. If only one project exists, run `npx --yes n8nac@v1 init-project --project-index 1 --sync-folder workflows`. If multiple projects exist, ask the user which one to use, then run `npx --yes n8nac@v1 init-project --project-id <id> [--sync-folder <path>]`.
...
- ✅ Run `npx --yes n8nac@v1 skills node-info <nodeName>` before writing parameters.
...
3. If not initialized, ask the user for the host URL and API key if needed, then run `npx --yes n8nac@v1 init-auth` and `npx --yes n8nac@v1 init-project` yourself.
4. Pull the workflow before any modification and show the command.
5. For a new workflow, read `workflowDir` from the active instance in `n8nac-config.json` to find the correct directory. In the common case it is workspace-relative, but it can also be absolute if `syncFolder` is absolute. Create the file there and confirm it appears in `npx --yes n8nac@v1 list --local` before pushing.
...
11. For webhook/chat/form workflows: run `npx --yes n8nac@v1 test-plan <id>` after pushing to inspect trigger, endpoints, and suggested payload.
    - Then run `npx --yes n8nac@v1 test <id>` with the inferred payload when runtime validation is needed.
